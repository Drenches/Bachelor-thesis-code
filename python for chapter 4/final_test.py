# -*- coding: utf-8 -*-
"""
Created on Tue May 19 18:57:05 2020
...
"""
Created on Sat May 16 10:47:45 2020

@author: jingyi li

DQN for blockchain applied in edge computing
...
class ENV(object):
    def __init__(self):
        ...
        self.transitionMatrix = [[0.4, 0.3, 0.2, 0.1],
                                 ...
                                 [0.1, 0.2, 0.3, 0.4]]
    def reset(self):
        re_env = rm.sample(self.transitionStates, 3)
        return re_env
        
    def update(self, x): # x是一个1*3的矩阵,有限马尔科夫过程
        new_env = [0] * N_STATES
    ...
    def feedback(self, s, a_aciton):
        ...
        delta3 = theta
        T3 = delta3/F_mis
        # 开始一个PBFT过程，client发出request, 不需要reply
...
class DQN(object):
    ...
    def learn(self):
        ...
        q_target = b_r + gamma * q_next.max(1)[0].view(U, 1)   # shape (batch, 1)
...
for i_episode in range(E_max):
    ...
    for t in range(H_max):
        ...
        r = 1/tc
        term_reward = np.append(term_reward, r)
        moving_counter += 1
        if moving_counter == 1000:
            moving_ave_eq_re = np.append(moving_ave_eq_re, np.mean(term_reward))
            term_reward = np.zeros(1)
            moving_counter = 0
        while True:
        ...
        if dqn.memory_counter > replay_buffer:
            dqn.learn()
        s = s_
plt.plot(moving_ave_eq_re*2.7428)
plt.show()
